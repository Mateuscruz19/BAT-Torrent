...
- [~] **② Search page** — 1:1 port of `SearchWindow` content into the `SearchView.qml` page landed (search bar, source/category selectors, results list w/ repacker chips, footer + game-source manager), reusing `QmlSearchBridge`. Rail/toolbar/menu/empty-state now navigate to page 2 instead of opening the window; `searchWinLoader` removed (`SearchWindow.qml` now orphaned in qrc — delete in a cleanup pass). **Remaining (redesign, decide first):** poster-grid layout + complex filters (repacker/size/seeders/source/category).
