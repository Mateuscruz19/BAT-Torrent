...
QString ReleaseNotesDialog::releaseNotes()
{
    return QStringLiteral(
        ...
        "<li><b>Discord Rich Presence</b> &mdash; shows &quot;Downloading X &middot; 67%&quot; in "
        "your Discord profile activity, with the elapsed timer running from app launch. "
        "Implemented as a pure Discord IPC client (no SDK dependency); the user supplies "
        "their own Discord application ID from the developer portal.</li>"
        "<li><b>Smart Paste (Ctrl+V)</b> &mdash; clipboard with a magnet link or 40-char "
        "info hash triggers a quick &quot;Add this torrent?&quot; prompt. Skips the entire "
        "File &rarr; Add Magnet flow for the most common workflow.</li>"
        "<li><b>Torrent Inspector</b> &mdash; <i>File &rarr; Inspect .torrent...</i> opens a "
        "preview dialog (name, size, file list, trackers, info hash, creator, comment, "
        "private flag) without adding the torrent. Add-now button promotes to a real "
        "add via the existing save-path flow.</li>"
        "<li><b>Multi-tag system</b> &mdash; right-click &rarr; <i>Tags...</i> assigns "
        "multiple free-form tags per torrent (in addition to the single Category). "
        "Persisted per info-hash, survives restart.</li>"
        "<li><b>Force Start</b> &mdash; context menu toggle to exempt a torrent from the "
        "active-downloads queue cap. The one torrent you actually need now stays running "
        "regardless of the limit.</li>"
        "<li><b>Recently removed</b> &mdash; <i>File &rarr; Recently removed...</i> lists the "
        "last 50 deleted torrents with timestamps and offers one-click restore. Resume "
        "snapshots are stashed under <code>&lt;AppData&gt;/removed/</code> when a torrent is "
        "removed; files on disk are untouched (you used Remove, not Remove with files).</li>"
        "<li><b>Full Backup / Restore</b> &mdash; <i>Settings &rarr; Full backup...</i> bundles "
        "every setting + every <code>.resume</code> file into a single <code>.bat</code> archive "
        "for migration between machines.</li>"
        "<li><b>Native OS notifications</b> &mdash; download-complete events also fire "
        "<code>QSystemTrayIcon::showMessage</code> so they land in macOS Notification Center, "
        "Windows Action Center, and Linux D-Bus notifications. Visible even when the app "
        "is on another desktop.</li>"
        "<li><b>Resume on network change</b> &mdash; <code>QNetworkInformation</code> watcher "
        "force-reannounces every torrent the moment the system reports it's back online "
        "(laptop wake / Wi-Fi reconnect). No more 10-minute wait for the next scheduled "
        "tracker probe.</li>"
        "<li><b>Changelog popup after update</b> &mdash; first launch after a version bump "
        "auto-opens the release notes. macOS / Windows convention.</li>"
        "<li><b>Inline rename (F2)</b> &mdash; the universal file-manager rename keystroke "
        "now works on the selected torrent row.</li>"
        "<li><b>Discord community link</b> &mdash; About dialog includes a direct link to "
        "the maintainer's Discord for support and feedback.</li>"
        "<li><b>i18n sweep</b> &mdash; all 80 new v2.5.0 strings translated to EN, PT, "
        "ZH, JA, RU, ES, DE (622 keys &times; 7 languages, 100% coverage).</li>"
        "</ul>"
        "<h3>v2.4.2</h3>"
        "<ul>"
